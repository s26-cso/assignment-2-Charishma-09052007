...
# Output: integers separated by exactly one space, newline at end, no trailing space.
...
.Linit_done:
    ...
    # ── monotone stack sweep right to left ────────────────────────
    ld      s1, -8(s0)
    addi    s1, s1, -1          # i = n-1
.Lsweep:
    bltz    s1, .Lsweep_done
...
.Lsweep_done:
    ...
    # ── print: number space number space ... number newline ────────
    # print space BEFORE each element except the first
    li      s1, 0
.Lprint:
    ld      t0, -8(s0)
    bge     s1, t0, .Lprint_done

    # print space before (not before first element)
    beqz    s1, .Lprint_num
    la      a0, fmt_sp
    call    printf
